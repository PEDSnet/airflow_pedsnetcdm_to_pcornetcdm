-- extracts conditions
begin;
create table {{ dag_run.conf['site'] }}_pcornet_airflow.filter_condition as
select co.* 
from 
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet_airflow.condition_transform as
select 
...
from 
	{{ dag_run.conf['site'] }}_pcornet_airflow.filter_condition co
join 
...
begin;
update {{ dag_run.conf['site'] }}_pcornet_airflow.condition_transform
set 
...
from 
	{{ dag_run.conf['site'] }}_pcornet_airflow.condition_transform d
inner join 
...
where 
	...
	and {{ dag_run.conf['site'] }}_pcornet_airflow.condition_transform.conditionid = d.conditionid 
	and {{ dag_run.conf['site'] }}_pcornet_airflow.condition_transform.condition_type in ('SM','OT');
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet_airflow.filter_obs_deriv as
select co.* 
from 
	{{ dag_run.conf['site'] }}_pedsnet.observation_derivation_covid co
inner join 
	{{ dag_run.conf['site'] }}_pcornet_airflow.person_visit_start2001 pvs 
	on co.person_id = pvs.person_id
where 
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet_airflow.obs_deriv_transform as
select 
	...
	'{{ dag_run.conf['site'] }}' as site
from 
	{{ dag_run.conf['site'] }}_pcornet_airflow.filter_obs_deriv;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet_airflow.obs_chief_compl_transform as
select 
	distinct on (observation_id)('o'|| observation_id)::text as conditionid,
...
where 
	...
	and obs.site = '{{ dag_run.conf['site'] }}'
	and EXTRACT(YEAR FROM observation_date) >= 2001
;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet_airflow.cond_chief_compl_transf as
select 
...
begin;
insert into {{ dag_run.conf['site'] }}_pcornet_airflow.condition(
...
select ('c'||conditionid) as conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet_airflow.condition_transform
union
select conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet_airflow.obs_deriv_transform
union 
select conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet_airflow.obs_chief_compl_transform
union
select conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet_airflow.cond_chief_compl_transf; 
commit;	

begin;
CREATE INDEX idx_cond_encid2 ON {{ dag_run.conf['site'] }}_pcornet_airflow.condition (encounterid);
commit;

begin;		
delete from {{ dag_run.conf['site'] }}_pcornet_airflow.condition 
where
