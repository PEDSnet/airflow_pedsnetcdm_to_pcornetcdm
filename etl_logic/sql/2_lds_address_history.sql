...
INSERT INTO {{ dag_run.conf['site'] }}_pcornet_airflow.lds_address_history(
...
select distinct
	case 
	    ...
	    when loc.city ~ '[^[:alnum:] ]' then 
			case 
				when regexp_replace(loc.city, '[^\w]+','') = '' then null 
		        else regexp_replace(loc.city, '[^\w]',' ') 
			end 
		else loc.city 
	end as address_city, 
	loc.county as address_county, 
	...
	'HO' as address_use, 
	case 
		when length(loc.zip) = 5 and zip ~ '^[0-9]+$' then loc.zip 
		when length(loc.zip) > 5 and replace(loc.zip,'-','') ~ '^[0-9]+$' then left(replace(loc.zip,'-',''),5)
		else null 
	end as address_zip5, 
	case 
		when length(replace(loc.zip,'-',''))= 9 and replace(loc.zip,'-','') ~ '^[0-9]+$' then left(replace(loc.zip,'-',''),9) 
		else null 
	end as address_zip9, 
...
from 
	{{ dag_run.conf['site'] }}_pedsnet.location loc 
left join 
	{{ dag_run.conf['site'] }}_pedsnet.location_history loc_his 
...
left join 
	...
	and addr_pref.source_concept_class = 'address_preferred'
left join 
	pcornet_maps.pedsnet_pcornet_valueset_map addr_typ 
	on addr_typ.source_concept_id::int = loc_his.relationship_type_concept_id 
	and addr_typ.source_concept_class = 'address_type'
left join 
	pcornet_maps.pedsnet_pcornet_valueset_map addr_st 
	on 
		case 
			when length(loc.state) = 2 then lower(trim(split_part(addr_st.pcornet_name,'=',1))) = lower(loc.state) 
