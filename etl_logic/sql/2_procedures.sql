...
insert into {{ dag_run.conf['site'] }}_pcornet_airflow.procedures(
...
select distinct
	...
	cast(po.person_id as text) as patid,
	...
	coalesce(
		...
		case 
			when left(split_part(procedure_source_value,'|',2),11) = ' ' or left(split_part(procedure_source_value,'|',2),11) = '' then 'NI'
...
from 
	{{ dag_run.conf['site'] }}_pedsnet.procedure_occurrence po
inner join 
	{{ dag_run.conf['site'] }}_pcornet_airflow.person_visit_start2001 pvs 
	on po.person_id = pvs.person_id
	and po.visit_occurrence_id = pvs.visit_id
join 
	{{ dag_run.conf['site'] }}_pcornet_airflow.encounter enc 
	on cast(po.visit_occurrence_id as text)=enc.encounterid
left join 
	vocabulary.concept px_cd_1 
	on px_cd_1.concept_id = po.procedure_concept_id 
	and px_cd_1.vocabulary_id in ('HCPCS','CPT4','ICD10PCS','ICD9Proc')
left join 
	vocabulary.concept px_cd_2 
	on px_cd_2.concept_id = po.procedure_source_concept_id 
	and px_cd_2.vocabulary_id in ('HCPCS','CPT4','ICD10PCS','ICD9Proc')
left join 
	pcornet_maps.pedsnet_pcornet_valueset_map px_typ 
 	on 
		case 
...
left join 
	...
	on cast(po.procedure_type_concept_id as text) = m4.source_concept_id 
	AND m4.source_concept_class='px source'
left join 
	pcornet_maps.pedsnet_pcornet_valueset_map m5 
	on cast(po.procedure_type_concept_id as text) = m5.source_concept_id 
	AND m5.source_concept_class='ppx'                                                                
where 
	EXTRACT(YEAR FROM procedure_date) >= 2001
...
select
	...
	cast(imm.person_id as text) as patid,
	cast(visit_occurrence_id as text) as encounterid,
	enc.enc_type as enc_type,
	enc.admit_date as admit_date,
	enc.providerid as providerid,
	...
	'{{ dag_run.conf['site'] }}' as site
from 
	{{ dag_run.conf['site'] }}_pedsnet.immunization imm
inner join 
	{{ dag_run.conf['site'] }}_pcornet_airflow.person_visit_start2001 pvs 
	on imm.person_id = pvs.person_id
	and imm.visit_occurrence_id = pvs.visit_id  
join 
	{{ dag_run.conf['site'] }}_pcornet_airflow.encounter enc
	on cast(imm.visit_occurrence_id as text)=enc.encounterid                                                        
where  
...
begin;
delete from {{ dag_run.conf['site'] }}_pcornet_airflow.procedures
where 
	(length(px) != 7  and px_type = '10')
	or 
	(length(px) < 5  and px_type = 'CH')
;
commit;
