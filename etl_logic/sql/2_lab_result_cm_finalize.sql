...
insert into {{ dag_run.conf['site'] }}_pcornet_airflow.lab_result_cm (
	lab_result_cm_id,
	patid, encounterid,
...
from {{ dag_run.conf['site'] }}_pcornet_airflow.lab_qual m
where m.encounterid::int IN (select visit_id from {{ dag_run.conf['site'] }}_pcornet_airflow.person_visit_start2001);
