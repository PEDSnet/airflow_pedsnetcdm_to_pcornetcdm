begin;
create table {{ dag_run.conf['site'] }}_pcornet.filter_diag as
select co.* 
from 
...
CREATE INDEX idx_filtdia_encid
    ON {{ dag_run.conf['site'] }}_pcornet.filter_diag USING btree
    (visit_occurrence_id )
...
begin;
insert into {{ dag_run.conf['site'] }}_pcornet.diagnosis(
...
from 
	{{ dag_run.conf['site'] }}_pcornet.filter_diag co
join 
	vocabulary.concept c2 on co.condition_concept_id = c2.concept_id
join 
	{{ dag_run.conf['site'] }}_pcornet.encounter enc on cast(co.visit_occurrence_id as text)=enc.encounterid
left join 
...
begin;
delete from {{ dag_run.conf['site'] }}_pcornet.diagnosis
where length(dx) < 2;
commit;

begin;
update {{ dag_run.conf['site'] }}_pcornet.diagnosis
set 
	dx = v.concept_code, 
	dx_type = case 
...
from 
	{{ dag_run.conf['site'] }}_pcornet.diagnosis d
inner join 
...
where 
	d.dx_type in ('SM','OT') 
	and {{ dag_run.conf['site'] }}_pcornet.diagnosis.diagnosisid = d.diagnosisid 
	and {{ dag_run.conf['site'] }}_pcornet.diagnosis.dx_type in ('SM','OT')
;
commit;
