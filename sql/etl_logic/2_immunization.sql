begin;
create table {{ dag_run.conf['site'] }}_pcornet.imm_code as
select distinct
...
left join 
	{{ dag_run.conf['site'] }}_pcornet.procedures proc 
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.imm_dose as
select 
	immunizationid,
	patid,
	encounterid,
	...
	site
from 
	{{ dag_run.conf['site'] }}_pcornet.imm_code imm																																				 
left join 
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.imm_code;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.imm_route as
select 
	immunizationid,
	patid,
	encounterid,
	...
	site
from 
	{{ dag_run.conf['site'] }}_pcornet.imm_dose imm																																				 
left join 
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.imm_dose;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.imm_manuf as
select 
	immunizationid,
	patid,
	encounterid,
	...
	site
from 
	{{ dag_run.conf['site'] }}_pcornet.imm_route imm																																				 
left join 
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.imm_route;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.imm_body_site as
select 
	immunizationid,
	patid,
	encounterid,
...
from 
	{{ dag_run.conf['site'] }}_pcornet.imm_manuf imm																																				 
left join 
...
begin;
drop table if exists {{ dag_run.conf['site'] }}_pcornet.imm_manuf;
commit;

begin;
INSERT INTO {{ dag_run.conf['site'] }}_pcornet.immunization(immunizationid, patid, encounterid, proceduresid, vx_providerid,
...
select distinct on (immunizationid)imm.immunizationid as immunizationid,
 ...
 from {{ dag_run.conf['site'] }}_pcornet.imm_body_site imm																																				 
 where imm.patid::int IN (select person_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);
 commit;

 begin;
 delete from {{ dag_run.conf['site'] }}_pcornet.immunization 
 where encounterid IS NOT null and
 	   encounterid::int NOT IN (select visit_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);
 commit;
