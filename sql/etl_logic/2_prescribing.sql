begin;
ALTER TABLE {{ dag_run.conf['site'] }}_pcornet.prescribing ALTER raw_rxnorm_cui SET DATA TYPE character varying(20);
alter table {{ dag_run.conf['site'] }}_pcornet.prescribing alter rxnorm_cui SET DATA TYPE character varying(8);
ALTER TABLE {{ dag_run.conf['site'] }}_pcornet.prescribing ALTER rx_quantity SET DATA TYPE NUMERIC(20,2);
ALTER TABLE {{ dag_run.conf['site'] }}_pcornet.prescribing ALTER rx_refills SET DATA TYPE NUMERIC(20,2);
ALTER TABLE {{ dag_run.conf['site'] }}_pcornet.prescribing ALTER rx_days_supply SET DATA TYPE NUMERIC(20,2);
ALTER TABLE {{ dag_run.conf['site'] }}_pcornet.prescribing ALTER rx_dose_ordered SET DATA TYPE NUMERIC(20,2);
commit;
--drop table {{ dag_run.conf['site'] }}_pcornet.rx_dose_form_data ;

begin;
create  table {{ dag_run.conf['site'] }}_pcornet.rx_dose_form_data
as
(
  select 
  	distinct de.drug_concept_id, 
  ...
  where 
  	de.drug_type_concept_id IN (38000177, 38000180,581373)
...
CREATE INDEX idx_drug_concept_id ON {{ dag_run.conf['site'] }}_pcornet.rx_dose_form_data (drug_concept_id);
commit;

begin;
insert into {{ dag_run.conf['site'] }}_pcornet.prescribing (prescribingid,
            patid, encounterid,
...
select 
	distinct on (drug_exposure_id) drug_exposure_id::text as prescribingid,
	...
	LPAD(date_part('hour',drug_exposure_start_datetime)::text,2,'0')||':'||LPAD(date_part('minute',drug_exposure_start_datetime)::text,2,'0') as rx_order_time,
	...
	case 
		when (c1.concept_name is null) then split_part(drug_source_value,'|',1) --- extract from drug source value
...
left join 
	{{ dag_run.conf['site'] }}_pcornet.rx_dose_form_data rdf 
...
left join pcornet_maps.pedsnet_pcornet_valueset_map m2 
	on cast(rdf.rx_dose_form_concept_id as text) = m2.source_concept_id 
	and m2.source_concept_class='Rx Dose Form'
left join pcornet_maps.pedsnet_pcornet_valueset_map m3 
	on cast(trim(lower(de.frequency)) as text) = m3.source_concept_id 
	and m3.source_concept_class='Rx Frequency'
left join pcornet_maps.pedsnet_pcornet_valueset_map m4 
	on cast(de.route_concept_id as text) = m4.source_concept_id 
	and m4.source_concept_class='Route'
left join pcornet_maps.pedsnet_pcornet_valueset_map m5 
	on cast(de.dispense_as_written_concept_id as text) = m5.source_concept_id  
...
where 
	...
	from {{ dag_run.conf['site'] }}_pedsnet.visit_occurrence V 
