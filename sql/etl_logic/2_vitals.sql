begin;
ALTER TABLE {{ dag_run.conf['site'] }}_pcornet.vital ALTER original_bmi SET DATA TYPE NUMERIC(20,8);
commit;
begin;
create sequence IF NOT EXISTS {{ dag_run.conf['site'] }}_pcornet.sq_vitalid start 1;
commit;

-- extract all fields 
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ms_ht as
(
    select  distinct person_id, '{{ dag_run.conf['site'] }}' as site, measurement_id,visit_occurrence_id, measurement_date, measurement_datetime, value_as_number
	...
	from {{ dag_run.conf['site'] }}_pedsnet.measurement
...
CREATE INDEX idx_msht_mdtm
    ON {{ dag_run.conf['site'] }}_pcornet.ms_ht USING btree
...
commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ms_wt as
(
   select person_id, '{{ dag_run.conf['site'] }}' as site, measurement_id,visit_occurrence_id, measurement_date, measurement_datetime, value_as_number
	,  measurement_concept_id,measurement_source_value, provider_id,operator_concept_id, unit_concept_id, unit_source_value, value_as_concept_id,
	value_source_value,measurement_type_concept_id
	from {{ dag_run.conf['site'] }}_pedsnet.measurement
...
CREATE INDEX idx_mswt_mdtm
    ON {{ dag_run.conf['site'] }}_pcornet.ms_wt USING btree
    (measurement_datetime)
    TABLESPACE pg_default;
...
commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ms_bmi as
(
    select distinct person_id, '{{ dag_run.conf['site'] }}' as site, measurement_id,visit_occurrence_id, provider_id, value_as_concept_id,
...
begin;
    CREATE INDEX idx_msbm_msdtm
        ON {{ dag_run.conf['site'] }}_pcornet.ms_bmi USING btree
        (measurement_datetime)
        TABLESPACE pg_default;
...
commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ms_sys as
(
   select distinct person_id, '{{ dag_run.conf['site'] }}' as site, measurement_id, visit_occurrence_id, provider_id, value_as_concept_id,
		measurement_date, measurement_datetime, value_as_number,  measurement_concept_id,measurement_source_value,operator_concept_id, unit_concept_id, unit_source_value,
	value_source_value,measurement_type_concept_id
...
CREATE INDEX idx_mssy_mdtm
    ON {{ dag_run.conf['site'] }}_pcornet.ms_sys USING btree
    (measurement_datetime)
    TABLESPACE pg_default;
...
commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ms_dia as
(
    select distinct person_id, '{{ dag_run.conf['site'] }}' as site, measurement_id, provider_id, value_as_concept_id,
...
begin;
    CREATE INDEX idx_msdi_msdtm
        ON {{ dag_run.conf['site'] }}_pcornet.ms_dia USING btree
        (measurement_datetime)
        TABLESPACE pg_default;
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ms as
	...
	value_source_value,measurement_type_concept_id
	from {{ dag_run.conf['site'] }}_pcornet.ms_ht
	UNION
	select person_id, site, measurement_id, visit_occurrence_id, value_as_concept_id, measurement_date, measurement_datetime, value_as_number, measurement_concept_id, measurement_source_value, provider_id,operator_concept_id, unit_concept_id, unit_source_value,
	value_source_value,measurement_type_concept_id
	from {{ dag_run.conf['site'] }}_pcornet.ms_wt
	UNION
	select person_id, site, measurement_id, visit_occurrence_id, value_as_concept_id, measurement_date, measurement_datetime, value_as_number, measurement_concept_id, measurement_source_value, provider_id,operator_concept_id, unit_concept_id, unit_source_value,
	value_source_value,measurement_type_concept_id
	from {{ dag_run.conf['site'] }}_pcornet.ms_bmi
	UNION
	select person_id, site, measurement_id, visit_occurrence_id, value_as_concept_id, measurement_date, measurement_datetime, value_as_number, measurement_concept_id, measurement_source_value, provider_id,operator_concept_id, unit_concept_id, unit_source_value,
	value_source_value,measurement_type_concept_id
	from {{ dag_run.conf['site'] }}_pcornet.ms_sys
	UNION
	select person_id, site, measurement_id, visit_occurrence_id, value_as_concept_id, measurement_date, measurement_datetime, value_as_number, measurement_concept_id, measurement_source_value, provider_id,operator_concept_id, unit_concept_id, unit_source_value,
	value_source_value,measurement_type_concept_id
	from {{ dag_run.conf['site'] }}_pcornet.ms_dia;
commit;

begin;
delete from {{ dag_run.conf['site'] }}_pcornet.ms
where EXTRACT(YEAR FROM measurement_date) >= 2001 and
      ms.person_id not in (select person_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001) and
      ms.visit_occurrence_id not in (select visit_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);

commit;

begin;
    CREATE INDEX idx_ms_dtm
        ON {{ dag_run.conf['site'] }}_pcornet.ms USING btree
        (measurement_datetime)
        TABLESPACE pg_default;
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ob_tobacco as
(
	...
	from {{ dag_run.conf['site'] }}_pedsnet.observation o1
	left join pcornet_maps.pedsnet_pcornet_valueset_map m1 on cast(o1.value_as_concept_id as text) = m1.value_as_concept_id
	left join {{ dag_run.conf['site'] }}_pedsnet.fact_relationship f on o1.observation_id::bigint = f.fact_id_1
	where observation_concept_id IN ('4005823') and m1.source_concept_class = 'tobacco'
);
commit;
begin;
CREATE INDEX idx_tbc_factid
    ON {{ dag_run.conf['site'] }}_pcornet.ob_tobacco USING btree
    (fact_id_2)
    TABLESPACE pg_default;
commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ob_tobacco_type as
(
	...
	and EXTRACT(YEAR FROM observation_date) >= 2001 and
      person_id in (select person_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001) and
      visit_occurrence_id in (select visit_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001)
);
commit;
begin;
CREATE INDEX idx_toty_obsid
    ON {{ dag_run.conf['site'] }}_pcornet.ob_tobacco_type USING btree
    (observation_id)
    TABLESPACE pg_default;
commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ob_smoking as
(
	...
	from {{ dag_run.conf['site'] }}_pedsnet.observation o1 left join pcornet_maps.pedsnet_pcornet_valueset_map m3 on o1.value_as_concept_id::text = m3.value_as_concept_id
...
CREATE INDEX idx_tosmk_obsid
    ON {{ dag_run.conf['site'] }}_pcornet.ob_smoking USING btree
    (observation_id)
    TABLESPACE pg_default;

commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.ob_tobacco_data as
(
	...
	from {{ dag_run.conf['site'] }}_pcornet.ob_tobacco
	left join {{ dag_run.conf['site'] }}_pcornet.ob_tobacco_type on  ob_tobacco.fact_id_2 = ob_tobacco_type.observation_id
    left join {{ dag_run.conf['site'] }}_pcornet.ob_smoking on ob_tobacco.fact_id_2 = ob_smoking.observation_id
);

commit; 

--  drop table {{ dag_run.conf['site'] }}_pcornet.vital_extract;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.vital_extract_htwt as
select distinct
...
ms.site
FROM {{ dag_run.conf['site'] }}_pcornet.ms
left join {{ dag_run.conf['site'] }}_pcornet.ms_ht on ms.visit_occurrence_id = ms_ht.visit_occurrence_id
and ms.measurement_datetime = ms_ht.measurement_datetime 
left join {{ dag_run.conf['site'] }}_pcornet.ms_wt on ms.visit_occurrence_id = ms_wt.visit_occurrence_id
and ms.measurement_datetime = ms_wt.measurement_datetime 
left join {{ dag_run.conf['site'] }}_pcornet.ms_bmi on ms.visit_occurrence_id = ms_bmi.visit_occurrence_id
and ms.measurement_datetime = ms_bmi.measurement_datetime
;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.vital_extract_diasys as
select distinct
ms.person_id, ms.visit_occurrence_id, ms.measurement_date, ms.measurement_datetime,  
...
ms.site
FROM {{ dag_run.conf['site'] }}_pcornet.vital_extract_htwt ms
left join {{ dag_run.conf['site'] }}_pcornet.ms_sys on ms.visit_occurrence_id = ms_sys.visit_occurrence_id
and ms.measurement_datetime = ms_sys.measurement_datetime
left join {{ dag_run.conf['site'] }}_pedsnet.fact_relationship fr1 on fr1.fact_id_1 = ms_sys.measurement_id AND fr1.domain_concept_id_1=21 AND fr1.domain_concept_id_2=21
left join {{ dag_run.conf['site'] }}_pcornet.ms_dia on ms.visit_occurrence_id = ms_dia.visit_occurrence_id
and ms_dia.measurement_id= fr1.fact_id_2;
commit;

begin;
drop table {{ dag_run.conf['site'] }}_pcornet.vital_extract_htwt;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.vital_extract as
select distinct
ms.person_id, ms.visit_occurrence_id, ms.measurement_date, ms.measurement_datetime,  
...
ms.site
FROM {{ dag_run.conf['site'] }}_pcornet.vital_extract_diasys ms
left join {{ dag_run.conf['site'] }}_pcornet.ob_tobacco_data on ms.visit_occurrence_id = ob_tobacco_data.visit_occurrence_id
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.vital_extract_diasys;
commit;

begin;
DELETE from {{ dag_run.conf['site'] }}_pcornet.vital_extract
where person_id not in (select person_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);

DELETE from {{ dag_run.conf['site'] }}_pcornet.vital_extract
where visit_occurrence_id not in (select visit_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);
commit;

begin;
--- transform 
create table {{ dag_run.conf['site'] }}_pcornet.vital_transform as
SELECT distinct
...
site as site
FROM {{ dag_run.conf['site'] }}_pcornet.vital_extract
...
--- loading
begin;
insert into {{ dag_run.conf['site'] }}_pcornet.vital(
...
select nextval('{{ dag_run.conf['site'] }}_pcornet.sq_vitalid') as vitalid, patid, encounterid, measure_date, measure_time, vital_source,
            ht, wt, diastolic, systolic, original_bmi, bp_position, 
	    tobacco, tobacco_type, smoking
            ,raw_diastolic, raw_systolic, raw_bp_position,site
from {{ dag_run.conf['site'] }}_pcornet.vital_transform;
commit;
