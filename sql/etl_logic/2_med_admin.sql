...
insert into {{ dag_run.conf['site'] }}_pcornet.med_admin (medadminid,
            patid, encounterid,
...
left join 
	{{ dag_run.conf['site'] }}_pcornet.ndc_concepts ndc_via_source_concept 
	on ndc_via_source_concept.concept_id = drug_source_concept_id
left join 
	{{ dag_run.conf['site'] }}_pcornet.ndc_concepts ndc_via_source_value 
...
where
	...
	and de.drug_source_value not ilike any (array['%UNDILUTED DILUENT%','%KCAL/OZ%','%breastmilk%','%kit%','%item%','%formula%', '%tpn%','%custom%','%fat emulsion%','%parenteral nutrition%','%ZZBREAST MILK%','%FAT EMULSION%','%EMPTY BAG%']); 
commit;

begin;
create index med_admin_enc on {{ dag_run.conf['site'] }}_pcornet.med_admin (encounterid);
commit;
...
(
	select 
		drug_exposure_id 
	from 
		{{ dag_run.conf['site'] }}_pcornet.med_admin n
inner join 
	{{ dag_run.conf['site'] }}_pedsnet.drug_exposure de 
