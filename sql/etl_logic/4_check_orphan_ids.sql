...
begin
  for select_row in
	select 'select count(*) from {{ dag_run.conf['site'] }}_pcornet.' || tablename ||' where patid::int not in (select person_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);' as query
    from pg_tables
	...
	for select_row in
	select 'select count(*) from {{ dag_run.conf['site'] }}_pcornet.' || tablename ||' where encounterid is not null and encounterid::int not in (select visit_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);' as query
    from pg_tables
    where schemaname = '{{ dag_run.conf['site'] }}_pcornet'
