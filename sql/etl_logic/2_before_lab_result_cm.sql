...
-- alter table {{ dag_run.conf['site'] }}_pcornet.lab_result_cm  alter result_num SET DATA TYPE NUMERIC(25,8);
-- alter table {{ dag_run.conf['site'] }}_pcornet.lab_result_cm  alter result_unit SET DATA TYPE character varying(256);
-- commit;
begin;
create table {{ dag_run.conf['site'] }}_pcornet.lab_measurements as
(
  ...
  and m.visit_occurrence_id IN (select visit_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001)
  and EXTRACT(YEAR FROM m.measurement_date)>=2001
 );
commit;
begin;
CREATE INDEX idx_labms_visitid
    ON {{ dag_run.conf['site'] }}_pcornet.lab_measurements USING btree
    (visit_occurrence_id)
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.specimen_values as
...
from {{ dag_run.conf['site'] }}_pcornet.lab_measurements m
