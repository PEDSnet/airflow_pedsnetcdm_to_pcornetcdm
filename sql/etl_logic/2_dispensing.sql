...
create table {{ dag_run.conf['site'] }}_pcornet.rxnorm_ndc_crosswalk as
 (
    select 
 ...
 create table {{ dag_run.conf['site'] }}_pcornet.ndc_concepts as
 (
 ...
 insert into {{ dag_run.conf['site'] }}_pcornet.dispensing( dispensingid, patid, prescribingid, dispense_date,
...
left join 
	{{ dag_run.conf['site'] }}_pcornet.rxnorm_ndc_crosswalk 
	on drug_concept_id = rxnorm_concept_id
left join 
	{{ dag_run.conf['site'] }}_pcornet.ndc_concepts ndc 
...
where
	...
	and (
		...
		or split_part(drug_source_value,'|',1) in (select concept_code from {{ dag_run.conf['site'] }}_pcornet.ndc_concepts )
		)
