-- extracts conditions
begin;
create table {{ dag_run.conf['site'] }}_pcornet.filter_condition as
select co.* 
from 
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.condition_transform as
select 
...
from 
	{{ dag_run.conf['site'] }}_pcornet.filter_condition co
join 
...
begin;
update {{ dag_run.conf['site'] }}_pcornet.condition_transform
set 
...
from 
	{{ dag_run.conf['site'] }}_pcornet.condition_transform d
inner join 
...
where 
	...
	and {{ dag_run.conf['site'] }}_pcornet.condition_transform.conditionid = d.conditionid 
	and {{ dag_run.conf['site'] }}_pcornet.condition_transform.condition_type in ('SM','OT');
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.filter_obs_deriv as
select co.* 
from 
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.obs_deriv_transform as
select 
	...
	'{{ dag_run.conf['site'] }}' as site
from 
	{{ dag_run.conf['site'] }}_pcornet.filter_obs_deriv;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.obs_chief_compl_transform as
select 
	distinct on (observation_id)('o'|| observation_id)::text as conditionid,
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.cond_chief_compl_transf as
select 
...
begin;
insert into {{ dag_run.conf['site'] }}_pcornet.condition(
...
select ('c'||conditionid) as conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet.condition_transform
union
select conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet.obs_deriv_transform
union 
select conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet.obs_chief_compl_transform
union
select conditionid, patid, encounterid, report_date, resolve_date, onset_date,
        condition_status, condition, condition_type, condition_source,
        raw_condition_status, raw_condition, raw_condition_type, raw_condition_source,site
from {{ dag_run.conf['site'] }}_pcornet.cond_chief_compl_transf; 
commit;	

begin;
CREATE INDEX idx_cond_encid2 ON {{ dag_run.conf['site'] }}_pcornet.condition (encounterid);
commit;

begin;		
delete from {{ dag_run.conf['site'] }}_pcornet.condition 
where
