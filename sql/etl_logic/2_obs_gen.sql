begin;
create table {{ dag_run.conf['site'] }}_pcornet.filter_adt as
select * 
from {{ dag_run.conf['site'] }}_pedsnet.adt_occurrence adt
...
CREATE INDEX idx_filtadt_encid
    ON {{ dag_run.conf['site'] }}_pcornet.filter_adt USING btree
    (visit_occurrence_id )
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.adt_obs
as
...
adt.site
from {{ dag_run.conf['site'] }}_pcornet.filter_adt adt 
inner join {{ dag_run.conf['site'] }}_pcornet.encounter enc on enc.encounterid::int = adt.visit_occurrence_id and enc.admit_date = adt.adt_date;

commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.meas_obs_filt
as
...
from {{ dag_run.conf['site'] }}_pedsnet.measurement meas 
where meas.measurement_concept_id in (2000001422,4353936);

commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.meas_obs_qual
as
...
from {{ dag_run.conf['site'] }}_pcornet.meas_obs_filt meas 
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.meas_obs_filt;
commit;
...
(
	...
	from {{ dag_run.conf['site'] }}_pcornet.meas_obs_qual
	where obsgen_result_qual = 'OT'
	and value_source_value ~ '[a-z]'
)
update {{ dag_run.conf['site'] }}_pcornet.meas_obs_qual
...
where l.obsgenid = {{ dag_run.conf['site'] }}_pcornet.meas_obs_qual.obsgenid
and {{ dag_run.conf['site'] }}_pcornet.meas_obs_qual.obsgen_result_qual = 'OT';
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.meas_obs
as
...
from {{ dag_run.conf['site'] }}_pcornet.meas_obs_qual meas 
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.meas_obs_qual;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.device_obs_filt
as
...
dev.site
from {{ dag_run.conf['site'] }}_pedsnet.device_exposure dev
...
CREATE INDEX idx_devobsfilt_encid
    ON {{ dag_run.conf['site'] }}_pcornet.device_obs_filt USING btree
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.device_obs
as
...
from {{ dag_run.conf['site'] }}_pcornet.device_obs_filt dev
inner join {{ dag_run.conf['site'] }}_pcornet.encounter enc on enc.encounterid = dev.encounterid
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.census_block_group as
...
with person_location_id_dates as (
    select 
        ...
        end_date
    from 
        {{ dag_run.conf['site'] }}_pedsnet.location_history
    where 
...
person_current_location_no_dates as (
    ...
    from 
        {{ dag_run.conf['site'] }}_pedsnet.person p
    where 
        not exists 
            (
                select null 
...
from {{ dag_run.conf['site'] }}_pcornet.adt_obs
union 
...
from {{ dag_run.conf['site'] }}_pcornet.meas_obs
union 
select distinct on (obsgenid) obsgenid, encounterid, obsgen_abn_ind, obsgen_code, obsgen_id_modified, obsgen_providerid, obsgen_result_modifier, obsgen_result_num::numeric, obsgen_result_qual, obsgen_result_text, obsgen_result_unit, obsgen_source, obsgen_start_date, obsgen_start_time, obsgen_stop_date, obsgen_stop_time, obsgen_table_modified, obsgen_type, patid, raw_obsgen_code, raw_obsgen_name, raw_obsgen_result::text, raw_obsgen_type, raw_obsgen_unit, site 
from {{ dag_run.conf['site'] }}_pcornet.device_obs
union
select distinct on (obsgenid) obsgenid, encounterid, obsgen_abn_ind, obsgen_code, obsgen_id_modified, obsgen_providerid, obsgen_result_modifier, obsgen_result_num::numeric, obsgen_result_qual, obsgen_result_text, obsgen_result_unit, obsgen_source, obsgen_start_date, obsgen_start_time, obsgen_stop_date, obsgen_stop_time, obsgen_table_modified, obsgen_type, patid, raw_obsgen_code, raw_obsgen_name, raw_obsgen_result::text, raw_obsgen_type, raw_obsgen_unit, site 
from {{ dag_run.conf['site'] }}_pcornet.census_block_group;
commit;

begin;
delete from {{ dag_run.conf['site'] }}_pcornet.obs_gen
where patid::int not in (select person_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001);

delete from {{ dag_run.conf['site'] }}_pcornet.obs_gen
where (encounterid is not null
and encounterid::int not in (select visit_id from {{ dag_run.conf['site'] }}_pcornet.person_visit_start2001));
commit;

begin;
drop table {{ dag_run.conf['site'] }}_pcornet.adt_obs;
drop table {{ dag_run.conf['site'] }}_pcornet.meas_obs;
drop table {{ dag_run.conf['site'] }}_pcornet.device_obs;
drop table {{ dag_run.conf['site'] }}_pcornet.census_block_group;
commit;
