...
(select measurement_id, specimen_source_value
 from {{ dag_run.conf['site'] }}_pcornet.specimen_values
 where specimen_source = 'OT' or specimen_source is null
)
update {{ dag_run.conf['site'] }}_pcornet.specimen_values 
...
where m.measurement_id = {{ dag_run.conf['site'] }}_pcornet.specimen_values.measurement_id and {{ dag_run.conf['site'] }}_pcornet.specimen_values.specimen_source = 'OT' or {{ dag_run.conf['site'] }}_pcornet.specimen_values.specimen_source is null;
commit;

begin;
drop table {{ dag_run.conf['site'] }}_pcornet.lab_measurements;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.lab_priority_modif
as
...
from {{ dag_run.conf['site'] }}_pcornet.specimen_values m
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.specimen_values;
commit;

begin;
create table {{ dag_run.conf['site'] }}_pcornet.lab_unit as
select lab_result_cm_id,patid,encounterid,specimen_source,lab_result_source,lab_loinc_source,
	...
	raw_facility_code,
	site
from {{ dag_run.conf['site'] }}_pcornet.lab_priority_modif m
...
begin;
drop table {{ dag_run.conf['site'] }}_pcornet.lab_priority_modif;
commit;

begin;
CREATE INDEX idx_labms_valcptid
    ON {{ dag_run.conf['site'] }}_pcornet.lab_unit USING btree
    (value_as_concept_id)
...
begin;
create table {{ dag_run.conf['site'] }}_pcornet.lab_qual as
...
from {{ dag_run.conf['site'] }}_pcornet.lab_unit m
...
(
	...
	from {{ dag_run.conf['site'] }}_pcornet.lab_qual
	where result_qual = 'OT'
	and value_source_value ~ '[a-z]'
)
update {{ dag_run.conf['site'] }}_pcornet.lab_qual
...
where l.lab_result_cm_id = {{ dag_run.conf['site'] }}_pcornet.lab_qual.lab_result_cm_id
and {{ dag_run.conf['site'] }}_pcornet.lab_qual.result_qual = 'OT';
commit;

begin;
drop table {{ dag_run.conf['site'] }}_pcornet.lab_unit;
commit;
